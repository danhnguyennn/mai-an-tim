...
class threadToolTds(QThread):
            ...
            try:
                ...
                if self.session == 'FAIL':
                    ...
                    if self.getMail == 'file':
                        ...
                        file_w = open(f'data\\mail\\stream\\{self.folder}\\gmail.txt', 'w', encoding='utf-8')
                        for mail in self.arr_gmail:
                            file_w.write(mail.strip()+'\n')
                        file_w.close()
                    ...
                    for _ in range(4):
                        ...
                        for _ in range(2):
                            ...
                            self.adb.find_image('img\\thuong1.png', 1,threshold=0.7, row=self.row)
                ...
                if self.session == 'OK':
                    for _ in range(4):
                        ...
                        if self.adb.find_image('img\\tim.png', 1, click=False, row=self.row) or self.adb.find_image('img\\hs.png', 1, threshold=0.7, click=False, row=self.row):
                            for i in range(5):
                                if self.adb.find_image('img\\tkriengtu8.png', 5, threshold=0.6, click=False, row=self.row) or self.adb.find_image('img\\tkriengtu9.png', 3, threshold=0.7, click=False, row=self.row):
                    ...
                    for _ in range(20):
                        ...
                        try:
                            ...
                            if self.cache < 14:
                                ...
                                self.adb.runShell("input keyevent 4")
                                if self.adb.find_image('img\\follow8.png', 2, row=self.row):
                                    self.adb.runShell("input keyevent 4")
    ...
    def closeQcTiktok2(self):
        if '5200' in self.phone_device:
            ...
            self.adb.find_image('img\\tuchoi_8.png', 1, row=self.row)
        list_qc = ['closequa8', 'closevitri8', 'khongchophep', 'updatechplay', 'dahieu', 'xacnhan_nn', 'sendfeedback'] # thuong, thuong1
    ...
    def clearDataTiktok(self):
        ...
        if result.strip() == "Success":
            ...
            self.sendDataUpMainScreen.emit(self.dict_data)
            self.adb.runShell("svc wifi disable")
            sleep(3)
            self.adb.runShell("svc wifi enable")
            sleep(5)
            return True     
